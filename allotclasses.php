...
<form action="allotclasses.php" method="post" style="margin-top: 100px">
    ...
    <div align="center">
        <select name="course" class="list-group-item">
            ...
            ';
            <option value="2">CSE 2nd Year</option>
            <option value="3">CSE 3rd Year</option>
            <option value="4">CSE 4rth Year</option>
        </select>
    </div>
...
<div align="center">
    ...
    <table id=allotedclassroomstable>
        ...
        $q = mysqli_query(mysqli_connect("localhost", "root", "", "ttms"),
            "SELECT * FROM classrooms ");
        $courses = array('CSE 2nd Year', 'CSE 3rd Year', 'CSE 4th Year');
        while ($row = mysqli_fetch_assoc($q)) {
            if ($row['status'] == 0)
                continue;
